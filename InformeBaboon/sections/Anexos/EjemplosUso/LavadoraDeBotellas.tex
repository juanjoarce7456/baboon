...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{petri_lavadora_botellas}
...
\item \textbf{Definir la cantidad de Hilos de ejecución:}\\
            ...
            agruparlos en ComplexSequentialTaskControllers, que solo requieren
            ...
            \begin{itemize}
              ...
              \item \textbf{Task Controllers \emph{(A cargo del framework)}:}
                  ...
                  \item Devolver botellas limpias.
                  \end{enumerate}
              \item \textbf{Complex Sequential Task Controllers \emph{(A cargo
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{diagrama_clases_lavadora_botellas}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{topic_petri_relacion}
...
\section {Configuración del Entorno de Desarrollo con \nombreFramework
Framework}
...
\begin{enumerate}
   ...
   \item Importar Proyecto Maven:
        \begin{itemize}
          \item En Eclipse, ir a File -\textgreater Import\ldots -\textgreater
          Maven -\textgreater Existing Maven Projects. Luego seleccionar la
          carpeta \textit{/path/baboon\_examples/LavadoraBotellas}
        \end{itemize}
\end{enumerate}

\subsection* {Intellij}
\begin{enumerate}
    \item En Intellij, ir a File -\textgreater Open\ldots. Luego seleccionar el
    archivo
    \textit{/path/baboon\_examples/LavadoraBotellas/pom.xml}
    y hacer click en el botón ``Open as Project''
\end{enumerate}
...
\subsection* {Maven}
Desde una consola, ubicado en el directorio
\textit{/path/baboon\_examples/LavadoraBotellas} correr el comando:
\begin{itemize}
    \item mvn exec:java
\end{itemize}
La configuración de Maven escrita en el archivo \textit{pom.xml} especifica la
forma de ejecutar el proyecto. Gracias a esto resulta muy simple compilar y
...
\subsection* {Intellij}
Para ejecutar el proyecto desde Intellij, primero se deben realizas las
configuraciones necesarias:
\begin{itemize}
    \item Ir a Run -\textgreater Run\ldots -\textgreater Edit Configurations
    \item En la ventana Run hacer click sobre el signo '+' -\textgreater Maven
    \item Llenar los campos:
    \begin{itemize}
        \item Name: \textit{LavadoraBotellas}
        \item Working Directory:
        \textit{/path/baboon\_examples/LavadoraBotellas}
        \item Command Line: \textit{exec:java}
    \end{itemize}
    \begin{figure}[H]
    \centering
    \subfigure{\includegraphics[width=0.49\textwidth]{Ejemplo_intellij_run_config_01}}
    \subfigure{\includegraphics[width=0.49\textwidth]{Ejemplo_intellij_run_config_02}}
    \caption{Configuración para correr LavadoraBotellas desde IntelliJ}
    \end{figure}
\end{itemize}

Luego se ejecuta la aplicación con el botón \textit{Run}

\subsection* {Exportar a un Archivo Ejecutable}

Para generar un archivo \textit{.jar} ejecutable, se debe correr el siguientes
comando:
\begin{itemize}
    \item mvn package
\end{itemize}
El archivo resultante es
\textit{./target/LavadoraBotellas-0.0.1-SNAPSHOT-jar-with-dependencies.jar}
y para ejecutarlo basta con darle doble click, o desde una consola ejecutar:
\begin{itemize}
    \item java -jar ./target/LavadoraBotellas-0.0.1-SNAPSHOT-jar-with-dependencies.jar
\end{itemize}
